...
export function PhoneVerify({ onVerified, label }: Props) {
  const t = useT()
  ...
  const displayLabel = label ?? t('ss.contactPhoneLabel')
  ...
  async function handleGetCode() {
    ...
    setLoading(false)
    if (!res.ok) {
      setError(data.error ?? t('phone.codeError'))
      return
    }
    setCode(data.code)
  ...
  async function handleVerify() {
    ...
    setVerifying(false)
    if (!res.ok) {
      setError(data.error ?? t('phone.verifyError'))
      return
    }
    setStep('verified')
    ...
    return (
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2 rounded-md border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700">
          ...
          <span>{phone} — {t('phone.verified')}</span>
        </div>
      </div>
    )
  }
      ...
      <div className="flex gap-2">
        ...
        >
          {loading
            ...
            : step === 'sent' ? t('phone.retry') : t('phone.getCode')}
        </Button>
      </div>
      ...
      {step === 'sent' && code && (
        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-3">
          ...
          <div className="text-center py-1">
            ...
            {receivingNumber && (
              <p className="text-xs text-blue-600 mt-1.5">
                {t('phone.receivingNo')} <span className="font-semibold">{formatReceivingNumber(receivingNumber)}</span>
              </p>
            )}
          </div>
          ...
          <p className="text-xs text-blue-600 text-center leading-relaxed">
            {t('phone.smsOr').replace('{number}', formatReceivingNumber(receivingNumber))}
          </p>
          ...
          >
            ...
            {verifying ? t('phone.verifying') : t('phone.confirmVerify')}
          </Button>
        </div>
      )}
